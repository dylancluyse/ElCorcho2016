...
<div class="admin-edit">
    ...
    while ($row = $STH->fetch()) {
        ...
        echo 'Adres: ' . $klantadres . ', ' . $klantgemeente . '<br><br>';

        $STHbestellingen = $DBH->prepare('SELECT * FROM tblbestellingen WHERE klantid = :klantid');
        $STHbestellingen->bindParam(':klantid', $klantid);
        $STHbestellingen->execute();



        $aantal = $STHbestellingen->rowCount();

        if($aantal==0){
            echo 'Deze klant heeft nog niets besteld via onze site.';
        } else {
            echo 'Bestellingen: </br>';
            $STHbestellingen->setFetchMode(PDO::FETCH_OBJ);
            while($row = $STHbestellingen->fetch()){
                $bestellingid = $row->bestellingid;
                echo '<a href="showbestelling.php?bestellingid=' . $bestellingid .'">' . $bestellingid . '</a><br>';
            }
        }
        echo '</ul>';
    }
